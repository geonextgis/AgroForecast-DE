...
class CropFusionNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Ablation flags
        self.use_vsn = True # VSN is mandatory and should never be dropped
        ...
        # Static Selection (VSN always active)
        ...
        # Temporal Selection (VSN always active)
        ...
        if self.use_lstm:
            ...
            self.static_to_h = nn.Identity()
            self.static_to_c = nn.Identity()
            self.post_lstm_gate = nn.Identity()
            self.post_lstm_norm = nn.Identity()
        ...
        if self.use_static_enrichment:
            ...
            self.static_enrichment = nn.Identity()
        ...
        if self.use_attention:
            ...
            self.multihead_attn = nn.Identity()
            self.post_attn_gate = nn.Identity()
        ...
        if self.use_pyramidal_pooling:
            ...
            self.pyramidal_pooling = nn.Identity()
    ...
    def forward(self, x):
        ...
        if self.use_temporal_conv:
            temporal_features = self.temporal_conv(temporal_features, mask=valid_mask.to(self.device) if valid_mask is not None else None)
            
        # 3. LSTM Processing
        # ---------------------------------------------------------
        if self.use_lstm:
            batch_size = temporal_features.size(0)
            h0 = self.static_to_h(static_embedding)  
            h0 = h0.view(batch_size, self.lstm_layers, self.hidden_size).permute(1, 0, 2).contiguous()
            c0 = self.static_to_c(static_embedding)
            c0 = c0.view(batch_size, self.lstm_layers, self.hidden_size).permute(1, 0, 2).contiguous()
        ...
        if self.use_attention:
            ...
            row_mask = valid_mask.unsqueeze(-1) if valid_mask is not None else 1
        ...
        if self.use_pyramidal_pooling:
            pooled_output, pooled_weights = self.pyramidal_pooling(
                attn_output, ~valid_mask.bool() if valid_mask is not None else None
            )
        else:
            # Fallback: Mean pooling over time dimension to remove sequence length
            if valid_mask is not None:
                # Mask out padded elements before averaging
                mask_expanded = valid_mask.unsqueeze(-1).float()
                sum_embeddings = torch.sum(attn_output * mask_expanded, dim=1)
                valid_counts = torch.clamp(mask_expanded.sum(dim=1), min=1.0)
                pooled_output = sum_embeddings / valid_counts
            else:

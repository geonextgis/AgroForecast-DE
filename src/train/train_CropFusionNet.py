...
def parse_args():
    ...
    parser.add_argument("--weight_decay", type=float, help="AdamW Weight Decay")
    ...
    # 3. Ablation Flags
    parser.add_argument("--use_lstm", type=lambda x: (str(x).lower() == 'true'), default=True)
    parser.add_argument("--use_attention", type=lambda x: (str(x).lower() == 'true'), default=True)
    parser.add_argument("--use_temporal_conv", type=lambda x: (str(x).lower() == 'true'), default=True)
    parser.add_argument("--use_static_enrichment", type=lambda x: (str(x).lower() == 'true'), default=True)
    parser.add_argument("--use_pyramidal_pooling", type=lambda x: (str(x).lower() == 'true'), default=True)
...
if __name__ == "__main__":
    ...
    # Ablation flags
    model_config["use_lstm"] = args.use_lstm
    model_config["use_attention"] = args.use_attention
    model_config["use_temporal_conv"] = args.use_temporal_conv
    model_config["use_static_enrichment"] = args.use_static_enrichment
    model_config["use_pyramidal_pooling"] = args.use_pyramidal_pooling
    ...
    print(f"   Weight Decay: {train_config.get('weight_decay', 1e-5)}")
    print(f"   Hidden: {model_config['lstm_hidden_dimension']}")
    print(f"   Batch: {train_config['batch_size']}")
    print(f"   Ablation: LSTM={args.use_lstm}, Attn={args.use_attention}, Conv={args.use_temporal_conv}, Enrich={args.use_static_enrichment}, Pool={args.use_pyramidal_pooling}")
    ...
    # 5. RUN TRAINING
    # ---------------------------------------------------------
    print("\n🏋️ Training model on full training set...")
    ...
    print(f"✅ Results saved to {save_path} | Best Val Loss: {best_val_loss:.4f}")

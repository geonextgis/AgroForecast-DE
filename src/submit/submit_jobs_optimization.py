...
output_base_dir = f"/beegfs/halder/GITHUB/RESEARCH/AgroForecast-DE/src/train/optimization/{CROP}/{FORECAST_MONTH}"
working_dir = "/beegfs/halder/GITHUB/RESEARCH/AgroForecast-DE/src/train"

# --- HYPERPARAMETER SEARCH SPACE ---
# Define the ranges you want to explore for hyperparameters and ablation flags
param_grid = {
    "lr": [1e-3, 1e-4, 1e-5],
    ...
    "dropout": [0.3, 0.4, 0.5],
    # Ablation flags - VSN is always retained and not included here
    "use_lstm": [True, False],
    "use_attention": [True, False],
    "use_temporal_conv": [True, False],
    "use_static_enrichment": [True, False],
    "use_pyramidal_pooling": [True, False],
}
...
def get_random_params():
    ...
    # Ensure VSN is always True, though it's not in the grid for sampling
    # params['use_variable_selection_network'] = True 
    return params
...
for i in range(NUM_JOBS):
    ...
    python_cmd = (
        ...
        f"--use_lstm {params['use_lstm']} "
        f"--use_attention {params['use_attention']} "
        f"--use_temporal_conv {params['use_temporal_conv']} "
        f"--use_static_enrichment {params['use_static_enrichment']} "
        f"--use_pyramidal_pooling {params['use_pyramidal_pooling']}"
    )
